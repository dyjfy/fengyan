...
( function() {
    ...
    var REMOTE = {
        ...
        // 弹框登录的提交地址
        poplogin: 'https://passport.lagou.com/login/login.json',

        // 弹窗登录中转
        poptransfer: 'https://passport.lagou.com/ajaxLogin/frameGrant.html',

        // 自动登录中转
        autologin: 'https://passport.lagou.com/ajaxLogin/login.html'
    ...
    function analyzeUri( config ) {
        ...
        // 如果为null，则说明是不加域名的绝对地址请求，例如：'https://passport.lagou.com/a/c.json'【不用跨域】
            ...
            $.ajax( config ).done( function( result, textStatus, jqXHR ){
                ...
                if ( result.state == '10001' ) {
                    ...
                    PASSPORT.remote(
                      ...
                      // 成功
                      function ( data ) {
                          util.tinfo( 'https://passport.lagou.com/static/js/passport.rpc.remote.succ' );
                          me( opt );
                      },
                      // 失败
                      function ( data ) {
                          util.tinfo( 'https://passport.lagou.com/static/js/passport.rpc.remote.fail' );
